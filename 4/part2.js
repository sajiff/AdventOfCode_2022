...
data.map((d) => {
  ...
  if (
    ...
    Number(firstElf.split("-")[0]) === Number(secondelf.split("-")[1]) ||
    Number(firstElf.split("-")[1]) === Number(secondelf.split("-")[0])
  ) {
    console.log("overlap");
    totalNumber = totalNumber + 1;
  } else if (
    Number(secondelf.split("-")[0]) <= Number(firstElf.split("-")[1]) &&
    Number(secondelf.split("-")[0]) >= Number(firstElf.split("-")[0])
  ) {
    console.log("overlap");
    totalNumber = totalNumber + 1;
  } else if (
    Number(secondelf.split("-")[1]) <= Number(firstElf.split("-")[1]) &&
    Number(secondelf.split("-")[1]) >= Number(firstElf.split("-")[0])
  ) {
    console.log("overlap");
    totalNumber = totalNumber + 1;
  } else {
